...
// GET /api/videos/fix-pdf-types
router.get('/fix-pdf-types', (req, res) => {
    try {
        run(`
            UPDATE videos
            SET type = 'pdf'
            WHERE type = 'video' AND (
                LOWER(title) LIKE '%.pdf' OR 
                LOWER(file_name) LIKE '%.pdf' OR
                LOWER(url) LIKE '%.pdf'
            )
        `)
        res.json({ success: true, message: 'Updated PDF types in-memory' })
    } catch (err) {
