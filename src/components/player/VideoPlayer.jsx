...
import { getNativeFile, requestRootHandle } from '../../utils/nativeFileSystem'
    ...
    */
    ...
    async function loadVideo() {
        ...
        try {
            ...
            if (video.filePath) {
                // Try native file first for smooth playback
                const nativeFile = await getNativeFile(video.filePath)
                if (nativeFile) {
                    console.log('Using native file system API for playback')
                    const url = URL.createObjectURL(nativeFile)
                    setVideoUrl(url)
                } else {
                    console.log('Fallback to server streaming')
                                        ...
                                        {settingsSubMenu === 'main' && (
                                            <div className="flex flex-col py-1">
                                                ...
                                                */}
                                                ...
                                                {/* Link Local Folder */}
                                                <div className="w-full px-4 py-2.5 flex items-center justify-between border-t border-white/5 mt-1 pt-2">
                                                    <span className="flex items-center gap-2" title="Enable buttery smooth playback by granting local folder access">
                                                        <FolderOpen className="w-4 h-4 opacity-75" /> Link Local Folder
                                                    </span>
                                                    <button
                                                        onClick={async () => {
                                                            await requestRootHandle()
                                                            loadVideo()
                                                            setShowSettingsMenu(false)
                                                        }}
                                                        className="text-xs bg-[var(--primary-bg)] hover:bg-white/10 px-2 py-1 rounded border border-white/10 transition-colors"
                                                    >
                                                        Link
                                                    </button>
                                                </div>
