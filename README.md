# UPSC Study Desk

A simple, offline-first personal study environment designed for focused learning.

## Features
- **Offline First**: All your data (videos, PDFs, notes) stays on your computer.
- **Study Mode**: Distraction-free interface for deep work.
- **Notes Durability**: Notes are saved to your local folder as Markdown files (`.md`).
- **Progress Tracking**: Manual completion tracking that you trust.

## How to use
1.  Open the website (or `index.html`).
2.  Click **"Select Master Folder"**.
3.  Choose the folder where you keep your study materials (e.g., `D:\UPSC Material`).
    -   *Structure*: `Paper` -> `Provider` -> `Course` -> `Files`
    -   *Example*: `GS1` -> `Vision IAS` -> `History` -> `Lecture 1.mp4`

## Hosting on GitHub (for yourself or friends)
1.  Upload (push) these files to a GitHub repository.
2.  Go to **Settings** > **Pages**.
3.  Under **Branch**, select `main` (or `master`) and click **Save**.
4.  Your site will be live at `https://your-username.github.io/repo-name/`.

**Note**: Hosting the site online does NOT upload your study files. It only hosts the *interface*. Use the "Select Master Folder" button to connect the online interface to your local files.
